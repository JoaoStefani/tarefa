...
class CawHelpers
{
    ...
    public static function formtData($data){
        $data = date('Y-m-d', strtotime($data));

        return $data;
    }

    public static function ImagemFormtBase64($caminho, $imegem){

        $file_data = $imegem;

        $file_name = 'imagem_'.time().rand().'.png'; //generating unique file name;

        @list($type, $file_data) = explode(';', $file_data);
        @list(, $file_data) = explode(',', $file_data);

        if($file_data!=""){ // storing image in storage/app/public Folder
            \Storage::disk('public')->put($caminho.'/'.$file_name,base64_decode($file_data));
        }
